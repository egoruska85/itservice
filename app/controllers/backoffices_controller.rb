class BackofficesController < ApplicationController
  ...
  def check_admin_access
    ...
    unless current_user.admin?
      session[:failed_admin_attempts] ||= 0
      session[:failed_admin_attempts] += 1

      if session[:failed_admin_attempts] >= 5
        session[:failed_admin_attempts] = 0
        ...
        remaining = 5 - session[:failed_admin_attempts]
        # Отправляем нарушителя на страницу организаций
        redirect_to root_path, alert: "Вам нельзя на страницу администратора." and return
      end
    end
